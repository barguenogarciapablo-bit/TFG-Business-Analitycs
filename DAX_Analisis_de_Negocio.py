...
print("="*70)
print("TFG DAX40: ANALISIS DE NEGOCIO - CARGANDO ARTEFACTOS")
print("="*70)
...
sil_score     = _art['sil_score'];     log_lik       = _art['log_lik']
...
print(f"    HMM Silhouette={sil_score:.4f}  Log-Likelihood={log_lik:.2f}")
...
def calcular_metricas_cartera(retornos_diarios):
    ...
    ret_neg = ret[ret < 0]
    downside_vol = np.std(ret_neg) * np.sqrt(252) if len(ret_neg) > 2 else np.std(ret) * np.sqrt(252)
...
for step in range(0, n_dias_test_total - PASO_REOPT, PASO_REOPT):
    ...
    ret_daily_equi.extend((ret_evaluacion.values @ pesos_equi).tolist())
    ...
    ret_daily_marko.extend((ret_evaluacion.values @ pesos_mk - coste / PASO_REOPT).tolist())
...
for step in range(0, n_dias_test_total - PASO_REOPT, PASO_REOPT):
    ...
    # Detectar regimen (soft weighting por probabilidades posteriores)
    ...
    _halflife_actual = int(sum(_proba_reg[r] * HALFLIFE_POR_REGIMEN[r] for r in range(k_optimo)))
    ...
    mu_ml = predecir_retornos_activos(
        ...
        seleccionadas, fecha_actual, model2=model_mlp, peso_model2=PESO_MLP
    )
    mu_hist = ret_estimacion.mean().values * 252
    ...
    coste = turnover * COSTE_TRANSACCION
    ret_daily_ml.extend((ret_evaluacion.values @ pesos_ml_opt - coste / PASO_REOPT).tolist())
...
if dax40_benchmark is not None:
    ...
    if len(dax40_test) > 1:
        ...
        ret_daily_dax40 = dax40_returns.values[:n_common_dax].tolist()
...
if mejora_sharpe <= 0:
    ...
    for iteracion, alpha_mult in enumerate([0.5, 0.3, 0.15, 0.0], 1):
        ...
        for step in range(0, n_dias_test_total - PASO_REOPT, PASO_REOPT):
            ...
            _halflife_iter = int(sum(_proba_iter[r] * HALFLIFE_POR_REGIMEN[r] for r in range(k_optimo)))
            ...
            ret_daily_ml_iter.extend((ret_evaluacion.values @ pesos_iter - turnover_iter * COSTE_TRANSACCION / PASO_REOPT).tolist())
...
n_common = min(len(ret_daily_equi), len(ret_daily_marko), len(ret_daily_ml))
...
print(f"  " + "="*95)
header = f"  {'Metrica':<25} {'Equiponderada':>15} {'Markowitz':>15} {'ML-Enhanced':>15}"
if metricas_dax40_f:
    header += f" {'DAX40 B&H':>15}"
print(header)
print(f"  " + "-"*95)
for m in metricas_nombres:
    line = f"  {m:<25}" + "".join(f" {v:>15.4f}" for v in [metricas_equi_f[m], metricas_marko_f[m], metricas_ml_f[m]])
    if metricas_dax40_f:
...
colores = {'Equiponderada': 'steelblue', 'Markowitz': 'darkgreen',
           'ML-Enhanced': 'darkorange', 'DAX40 B&H': 'purple'}

nombres    = ['Equipond.', 'Markowitz', 'ML-Enh.']
sharpes    = [metricas_equi_f['Sharpe Ratio'], metricas_marko_f['Sharpe Ratio'], metricas_ml_f['Sharpe Ratio']]
vols       = [metricas_equi_f['Volatilidad Anual (%)'], metricas_marko_f['Volatilidad Anual (%)'], metricas_ml_f['Volatilidad Anual (%)']]
rets_bar   = [metricas_equi_f['Retorno Anual (%)'], metricas_marko_f['Retorno Anual (%)'], metricas_ml_f['Retorno Anual (%)']]
dds        = [metricas_equi_f['Max Drawdown (%)'], metricas_marko_f['Max Drawdown (%)'], metricas_ml_f['Max Drawdown (%)']]
sortinos   = [metricas_equi_f['Sortino Ratio'], metricas_marko_f['Sortino Ratio'], metricas_ml_f['Sortino Ratio']]
cols_bar   = [colores['Equiponderada'], colores['Markowitz'], colores['ML-Enhanced']]
if metricas_dax40_f:
...
# Figura 22_a: Retornos acumulados comparativos de las 4 estrategias
...
def _barplot_metrica(data_vals, nombres_vals, cols_vals, titulo, ylabel, fmt, nombre_archivo):
    ...
    for i, v in enumerate(data_vals):
        offset = 0.02 if 'Drawdown' not in titulo else -0.5
        ax_bm.text(i, v + offset, fmt.format(v), ha='center', fontsize=10, fontweight='bold')
...
if pesos_ml_hist and fechas_ml_bt:
    pesos_df = pd.DataFrame(pesos_ml_hist[:len(fechas_ml_bt)], columns=seleccionadas,
                            index=pd.to_datetime(fechas_ml_bt[:len(pesos_ml_hist)]))
...
print(f"\n  RESULTADO CON {capital_ejemplo:,.0f} EUR INVERTIDOS:")
print(f"  " + "-"*60)
for nombre, metr in [("Equiponderada 1/N", metricas_equi_f),
                     ("Markowitz Clasico", metricas_marko_f),
                     ("ML-Enhanced", metricas_ml_f)]:
    ret_total = metr['Retorno Anual (%)'] * (n_common / 252)
    val_final = capital_ejemplo * (1 + ret_total / 100)
    print(f"  {nombre:<22} -> {val_final:>10,.0f} EUR  (ganancia: {val_final - capital_ejemplo:>+10,.0f} EUR)")
if metricas_dax40_f:
    ret_total_dax = metricas_dax40_f['Retorno Anual (%)'] * (n_common / 252)
    val_final_dax = capital_ejemplo * (1 + ret_total_dax / 100)
...
def _bootstrap_sharpe(rets_list, n_boot=5000, ci=0.95):
...
print(f"\n  Bootstrap: 5000 remuestreos, IC 95%\n")
...
# =============================================================================
# SECCION 22.C: WALK-FORWARD MULTI-VENTANA
# =============================================================================

print(f"\n" + "="*70)
print("SECCION 22.C: WALK-FORWARD MULTI-VENTANA — ROBUSTEZ TEMPORAL")
print("="*70)
...
_all_dates = retornos_full.index.sort_values()
_year_min  = _all_dates[0].year
_year_max  = _all_dates[-1].year

_wf_windows = []
for _yr_start in range(_year_min, _year_max - _WF_TRAIN_YEARS - _WF_TEST_YEARS + 2, _WF_STEP_YEARS):
    _tr_start = max(pd.Timestamp(f'{_yr_start}-01-01'), _all_dates[0])
    _tr_end   = pd.Timestamp(f'{_yr_start + _WF_TRAIN_YEARS}-01-01')
    _te_end   = min(pd.Timestamp(f'{_yr_start + _WF_TRAIN_YEARS + _WF_TEST_YEARS}-01-01'), _all_dates[-1])
    if _tr_end >= _te_end:
        continue
    _wf_windows.append((_tr_start, _tr_end, _te_end))

print(f"\n  Ventanas generadas: {len(_wf_windows)}")
for _i, (_ts, _te, _ee) in enumerate(_wf_windows):
    print(f"    V{_i+1}: train {_ts.date()} → {_te.date()} | test {_te.date()} → {_ee.date()}")

_wf_results = []
for _wf_i, (_wf_tr_start, _wf_tr_end, _wf_te_end) in enumerate(_wf_windows):
    _wf_ret_train = retornos_full.loc[(_wf_tr_start <= retornos_full.index) & (retornos_full.index < _wf_tr_end)]
    _wf_ret_test  = retornos_full.loc[(_wf_tr_end  <= retornos_full.index) & (retornos_full.index < _wf_te_end)]
    if len(_wf_ret_train) < 200 or len(_wf_ret_test) < 40:
        continue

    _wf_ret_full      = pd.concat([_wf_ret_train, _wf_ret_test]).sort_index()
    _wf_idx_test_start = len(_wf_ret_train)
    _wf_n_test        = len(_wf_ret_test)
    _wf_ret_equi, _wf_ret_marko, _wf_ret_ml = [], [], []
    _wf_prev_mk = np.ones(n_activos) / n_activos
    _wf_prev_ml = np.ones(n_activos) / n_activos

    for _wf_step in range(0, _wf_n_test - PASO_REOPT, PASO_REOPT):
        _wf_eval_start = _wf_idx_test_start + _wf_step
        _wf_eval_end   = min(_wf_eval_start + PASO_REOPT, len(_wf_ret_full))
        _wf_est_start  = max(0, _wf_eval_start - VENTANA_TRAIN_BT)
        _wf_est  = _wf_ret_full.iloc[_wf_est_start:_wf_eval_start]
        _wf_eval = _wf_ret_full.iloc[_wf_eval_start:_wf_eval_end]
        if len(_wf_est) < 60 or len(_wf_eval) == 0:
            continue
        _wf_fecha = _wf_ret_full.index[_wf_eval_start]

        _wf_ret_equi.extend((_wf_eval.values @ (np.ones(n_activos) / n_activos)).tolist())

        _wf_mu_h  = _wf_est.mean().values * 252
        _wf_sig_h = _wf_est.cov().values * 252
        _wf_p_mk  = optimizar_markowitz(_wf_mu_h, _wf_sig_h, n_activos, max_peso=1.0)
        _wf_to_mk = np.sum(np.abs(_wf_p_mk - _wf_prev_mk))
        _wf_ret_marko.extend((_wf_eval.values @ _wf_p_mk - _wf_to_mk * COSTE_TRANSACCION / PASO_REOPT).tolist())
        _wf_prev_mk = _wf_p_mk

        _wf_proba = np.ones(k_optimo) / k_optimo
        if _wf_fecha in regime_features.index:
            _wf_ridx  = regime_features.index.get_loc(_wf_fecha)
            _wf_proba = proba_regimenes[_wf_ridx]
        else:
            _wf_cidx  = max(0, min(regime_features.index.searchsorted(_wf_fecha) - 1, len(regimenes) - 1))
            _wf_proba = proba_regimenes[_wf_cidx]

        _wf_alpha    = float(sum(_wf_proba[r] * ALPHA_POR_REGIMEN[r] for r in range(k_optimo)))
        _wf_maxp     = float(sum(_wf_proba[r] * MAX_PESO_POR_REGIMEN[r] for r in range(k_optimo)))
        # Halflife walk-forward: media ponderada por probabilidades del regimen
        _wf_halflife = int(sum(_wf_proba[r] * HALFLIFE_POR_REGIMEN[r] for r in range(k_optimo)))

        _wf_mu_ml = predecir_retornos_activos(
            features_all, retornos_full, model_gb, scaler_features,
            seleccionadas, _wf_fecha, model2=model_mlp, peso_model2=PESO_MLP
        )
        _wf_gm    = _wf_mu_h.mean()
        _wf_sf    = max(0, 1 - (n_activos - 2) / (len(_wf_est) * np.sum((_wf_mu_h - _wf_gm)**2) + 1e-8))
        _wf_mu_enh = _wf_alpha * (_wf_mu_ml * 252) + (1 - _wf_alpha) * (_wf_gm + _wf_sf * (_wf_mu_h - _wf_gm))

        try:
            _wf_sig_ewma = _wf_est.ewm(halflife=_wf_halflife).cov().loc[_wf_est.index[-1]].values * 252
            if _wf_sig_ewma.shape != (n_activos, n_activos):
                raise ValueError()
            _wf_lw = LedoitWolf(); _wf_lw.fit(_wf_est.values)
            _wf_sig_enh = 0.70 * _wf_sig_ewma + 0.30 * _wf_lw.covariance_ * 252
        except Exception:
            _wf_lw = LedoitWolf(); _wf_lw.fit(_wf_est.values)
            _wf_sig_enh = _wf_lw.covariance_ * 252

        _wf_p_ml  = optimizar_markowitz(_wf_mu_enh, _wf_sig_enh, n_activos,
                                         max_peso=_wf_maxp, min_peso=MIN_PESO_ML)
        _wf_to_ml = np.sum(np.abs(_wf_p_ml - _wf_prev_ml))
        if _wf_to_ml > MAX_TURNOVER:
            _wf_p_ml = _wf_prev_ml + (_wf_p_ml - _wf_prev_ml) * (MAX_TURNOVER / _wf_to_ml)
            _wf_p_ml = np.maximum(_wf_p_ml, 0.0); _wf_p_ml /= _wf_p_ml.sum()
            _wf_to_ml = MAX_TURNOVER
        _wf_ret_ml.extend((_wf_eval.values @ _wf_p_ml - _wf_to_ml * COSTE_TRANSACCION / PASO_REOPT).tolist())
        _wf_prev_ml = _wf_p_ml

    if len(_wf_ret_equi) > 20 and len(_wf_ret_marko) > 20 and len(_wf_ret_ml) > 20:
        _n_wf = min(len(_wf_ret_equi), len(_wf_ret_marko), len(_wf_ret_ml))
        _wf_m_eq = calcular_metricas_cartera(_wf_ret_equi[:_n_wf])
        _wf_m_mk = calcular_metricas_cartera(_wf_ret_marko[:_n_wf])
        _wf_m_ml = calcular_metricas_cartera(_wf_ret_ml[:_n_wf])
        _wf_results.append({
            'ventana': f"V{_wf_i+1}",
            'train': f"{_wf_tr_start.year}-{_wf_tr_end.year}",
            'test':  f"{_wf_tr_end.year}-{_wf_te_end.year}",
            'sharpe_equi':  _wf_m_eq['Sharpe Ratio'],
            'sharpe_marko': _wf_m_mk['Sharpe Ratio'],
            'sharpe_ml':    _wf_m_ml['Sharpe Ratio'],
            'ret_equi':     _wf_m_eq['Retorno Anual (%)'],
            'ret_marko':    _wf_m_mk['Retorno Anual (%)'],
            'ret_ml':       _wf_m_ml['Retorno Anual (%)'],
            'ml_supera_marko': _wf_m_ml['Sharpe Ratio'] > _wf_m_mk['Sharpe Ratio'],
        })

print(f"\n  {'Ventana':<8} {'Train':<12} {'Test':<12} {'Sharpe EQ':>10} {'Sharpe MK':>10} {'Sharpe ML':>10} {'ML>MK':>7}")
print(f"  {'-'*8} {'-'*12} {'-'*12} {'-'*10} {'-'*10} {'-'*10} {'-'*7}")
_wf_ml_wins = 0
for _r in _wf_results:
    _wf_flag = 'SI' if _r['ml_supera_marko'] else 'NO'
    _wf_ml_wins += int(_r['ml_supera_marko'])
    print(f"  {_r['ventana']:<8} {_r['train']:<12} {_r['test']:<12} "
          f"{_r['sharpe_equi']:>10.4f} {_r['sharpe_marko']:>10.4f} "
          f"{_r['sharpe_ml']:>10.4f} {_wf_flag:>7}")

_wf_total = len(_wf_results)
_wf_pct   = (_wf_ml_wins / _wf_total * 100) if _wf_total > 0 else 0
print(f"\n  ML-Enhanced supera Markowitz en {_wf_ml_wins}/{_wf_total} ventanas ({_wf_pct:.0f}%)")
if _wf_total > 1:
    _wf_sharpes_ml = [r['sharpe_ml']    for r in _wf_results]
    _wf_sharpes_mk = [r['sharpe_marko'] for r in _wf_results]
    print(f"  Sharpe ML promedio:  {np.mean(_wf_sharpes_ml):.4f} +/- {np.std(_wf_sharpes_ml):.4f}")
    print(f"  Sharpe MK promedio:  {np.mean(_wf_sharpes_mk):.4f} +/- {np.std(_wf_sharpes_mk):.4f}")

if _wf_results:
    _wf_labels = [r['test'] for r in _wf_results]
    _wf_x = np.arange(len(_wf_labels))
    _wf_width = 0.25
    fig_wf, ax_wf = plt.subplots(figsize=(max(10, len(_wf_results) * 2), 6))
    ax_wf.bar(_wf_x - _wf_width, [r['sharpe_equi']  for r in _wf_results], _wf_width,
              label='Equiponderada', color='#90caf9', edgecolor='black', linewidth=0.3)
    ax_wf.bar(_wf_x,              [r['sharpe_marko'] for r in _wf_results], _wf_width,
              label='Markowitz',    color='#ffcc80', edgecolor='black', linewidth=0.3)
    ax_wf.bar(_wf_x + _wf_width,  [r['sharpe_ml']   for r in _wf_results], _wf_width,
              label='ML-Enhanced',  color='#a5d6a7', edgecolor='black', linewidth=0.3)
    ax_wf.set_xticks(_wf_x); ax_wf.set_xticklabels(_wf_labels, fontsize=10)
    ax_wf.set_xlabel('Periodo de Test', fontsize=11); ax_wf.set_ylabel('Sharpe Ratio', fontsize=11)
    ax_wf.set_title(f'Walk-Forward Multi-Ventana: Sharpe por Periodo de Test\n'
                    f'ML supera Markowitz en {_wf_ml_wins}/{_wf_total} ventanas ({_wf_pct:.0f}%)',
                    fontsize=12, fontweight='bold')
    ax_wf.legend(fontsize=10); ax_wf.axhline(0, color='gray', linestyle='--', linewidth=0.5)
    ax_wf.grid(alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_DIR, '22C_walk_forward_sharpe.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  [OK] Guardado: 22C_walk_forward_sharpe.png")
...
for _cost, _lbl in zip(_cost_scenarios, _cost_labels):
    ...
    for _s in range(0, n_dias_test_total - PASO_REOPT, PASO_REOPT):
        ...
        _cs_ret_mk.extend((_eval_blk.values @ _p_mk2 - _to_mk2 * _cost / PASO_REOPT).tolist())
        _cs_prev_mk2 = _p_mk2
    ...
    for _ti, _to_val in enumerate(turnover_hist):
        ...
        _n_days = PASO_REOPT if _ti < len(turnover_hist) - 1 else len(ret_daily_ml) - _daily_idx
        _n_days = min(_n_days, len(ret_daily_ml) - _daily_idx)
        for _d in range(_n_days):
            if _daily_idx + _d < len(ret_daily_ml):
                _cs_ret_ml2.append(ret_daily_ml[_daily_idx + _d] - _cost_diff_daily)
        _daily_idx += _n_days
...
print(f"""
  ...
  OBJETIVO 3: "Validar robustez temporal (Walk-Forward Multi-Ventana)"
  -----------------------------------------------------------------------
  {'  [ROBUSTO]' if _wf_total > 0 and _wf_pct >= 60 else '  [LIMITADO]'}

  Walk-forward con {_wf_total} ventanas ({_WF_TRAIN_YEARS}a train / {_WF_TEST_YEARS}a test):
    ML supera Markowitz en {_wf_ml_wins}/{_wf_total} ventanas ({_wf_pct:.0f}%)
    {'Sharpe ML promedio: ' + f"{np.mean(_wf_sharpes_ml):.4f} +/- {np.std(_wf_sharpes_ml):.4f}" if _wf_total > 1 else ''}
    {'Sharpe MK promedio: ' + f"{np.mean(_wf_sharpes_mk):.4f} +/- {np.std(_wf_sharpes_mk):.4f}" if _wf_total > 1 else ''}
  ...
  LIMITACIONES:
     - El backtest no incluye costes de financiacion ni impuestos
     - Supone ejecucion perfecta a precios de cierre
     - El walk-forward reutiliza modelos pre-entrenados (no re-entrena por ventana)
...
print(f"    22B_forest_plot_bootstrap_ci.png            - Bootstrap CI del Sharpe (forest plot)")
print(f"    22C_walk_forward_sharpe.png                 - Walk-Forward multi-ventana")
