...
print("="*70)
print("TFG DAX40: ANALISIS DEL DATO - Optimizador de carteras ML-Enhanced con detección de regímenes: aplicación al DAX40")
print("="*70)
...
print(f"       Kurtosis:  {_kurt_res:.4f}")
print(f"       Asimetría:  {_skew_res:.4f}")
...
# MODIFICACIÓN NUEVA: RESULTADOS NUMERICOS DEL DIAGNOSTICO DE RESIDUOS
# -------------------------------------------------
# ACF: lags significativos (|acf| > IC 95%)
_lags_sig_acf = [i for i in range(1, _nlags_acf + 1) if abs(_acf_res[i]) > _conf_acf]
_pct_sig_acf  = len(_lags_sig_acf) / _nlags_acf * 100

# Ljung-Box: resumen por lags clave
_lb_lags_clave = [5, 10, 20]
print(f"\n  [19.4] Resultados numericos del diagnostico de residuos:")
print(f"  {'='*65}")
print(f"\n  -- Estadisticos descriptivos de los residuos --")
print(f"    N obs:          {len(residuos)}")
print(f"    Media:          {residuos.mean():.6f}  (cercano a 0 = sin sesgo sistematico)")
print(f"    Desv. tipica:   {residuos.std():.6f}")
print(f"    Skewness:       {_skew_res:.4f}       (0 = simetrico)")
print(f"    Kurtosis:       {_kurt_res:.4f}       (0 = mesocurtica; >0 = colas gruesas)")
print(f"    QQ R:           {r_qq:.4f}            (1.0 = normal perfecta)")

print(f"\n  -- ACF de residuos ({_nlags_acf} lags, IC 95% = +/-{_conf_acf:.4f}) --")
print(f"    Lags significativos: {len(_lags_sig_acf)}/{_nlags_acf} ({_pct_sig_acf:.1f}%)")
if _lags_sig_acf:
    print(f"    Lags: {_lags_sig_acf}")
    print(f"    Nota: hasta ~5% de lags fuera del IC es esperado por azar (falsos positivos)")
else:
    print(f"    Ningun lag supera el IC 95% -> residuos consistentes con ruido blanco")

print(f"\n  -- Test Ljung-Box (H0: residuos son ruido blanco) --")
print(f"    {'Lag m':>6} {'Q-stat':>10} {'p-valor':>10} {'Resultado':>20}")
print(f"    {'-'*50}")
for m in _lb_lags_clave:
    idx = m - 1
    resultado = 'RECHAZA H0 (autocorr)' if _lb_p[idx] < 0.05 else 'No rechaza H0 (OK)'
    print(f"    {m:>6} {_lb_q[idx]:>10.3f} {_lb_p[idx]:>10.4f} {resultado:>20}")

# Veredicto global
_n_rechazo_lb = sum(1 for p in _lb_p if p < 0.05)
print(f"\n  -- Veredicto global --")
print(f"    Ljung-Box: {_n_rechazo_lb}/20 lags rechazan H0 al 5%")
if _n_rechazo_lb == 0 and len(_lags_sig_acf) <= 2:
    print(f"    -> Residuos compatibles con ruido blanco. Modelo captura bien la estructura.")
elif _n_rechazo_lb <= 3:
    print(f"    -> Autocorrelacion leve o marginal. Comun en series financieras de alta frecuencia.")
else:
    print(f"    -> Autocorrelacion significativa en residuos. Puede haber estructura no capturada.")

# GRAFICO 19.3e - Retornos reales vs predichos (media diaria sobre todos los tickers)
# Se agrupa por fecha calculando la media de retornos reales y predichos de todos
# los tickers del test set. Permite ver visualmente el seguimiento del modelo.
print(f"\n  [19.4e] Retornos predichos vs reales (media diaria, test set)...")

_ret_by_date = {}
for (ticker, date), y_r, y_p in zip(meta_test, y_test_all, y_pred_ensemble_test):
    _ret_by_date.setdefault(date, {'real': [], 'pred': []})
    _ret_by_date[date]['real'].append(y_r)
    _ret_by_date[date]['pred'].append(y_p)

_dates_sorted  = sorted(_ret_by_date.keys())
_ret_real_mean = pd.Series({d: np.mean(_ret_by_date[d]['real']) for d in _dates_sorted})
_ret_pred_mean = pd.Series({d: np.mean(_ret_by_date[d]['pred']) for d in _dates_sorted})

# Correlacion y RMSE de la serie media
_corr_media = np.corrcoef(_ret_real_mean.values, _ret_pred_mean.values)[0, 1]
_rmse_media = np.sqrt(np.mean((_ret_real_mean.values - _ret_pred_mean.values) ** 2))

fig_19_3e, axes_3e = plt.subplots(2, 1, figsize=(15, 9),
                                   gridspec_kw={'height_ratios': [3, 1]})
fig_19_3e.suptitle(
    'Retornos Reales vs Predichos — Ensemble (GB+MLP) | Test Set\n'
    f'Media diaria sobre todos los tickers  |  Correlacion={_corr_media:.4f}  |  RMSE={_rmse_media:.5f}',
    fontsize=12, fontweight='bold')

# Panel superior: lineas real vs predicho
ax_3e_top = axes_3e[0]
ax_3e_top.plot(_ret_real_mean.index, _ret_real_mean.values,
               color='steelblue', linewidth=1.2, alpha=0.85, label='Retorno real (media diaria)')
ax_3e_top.plot(_ret_pred_mean.index, _ret_pred_mean.values,
               color='darkorange', linewidth=1.2, alpha=0.85, label='Retorno predicho (Ensemble)')
ax_3e_top.axhline(0, color='black', linewidth=0.7, linestyle='--')
ax_3e_top.set_ylabel('Log-retorno diario', fontsize=10)
ax_3e_top.legend(fontsize=9)
ax_3e_top.grid(alpha=0.3)

# Panel inferior: error diario (real - predicho)
_error_diario = _ret_real_mean - _ret_pred_mean
_colores_err  = ['#e74c3c' if e < 0 else '#2ecc71' for e in _error_diario.values]
ax_3e_bot = axes_3e[1]
ax_3e_bot.bar(_error_diario.index, _error_diario.values,
              color=_colores_err, alpha=0.6, width=1.5, label='Error (real - predicho)')
ax_3e_bot.axhline(0, color='black', linewidth=0.8)
ax_3e_bot.set_ylabel('Error', fontsize=9)
ax_3e_bot.set_xlabel('Fecha', fontsize=10)
ax_3e_bot.legend(fontsize=8)
ax_3e_bot.grid(alpha=0.3)

plt.tight_layout()
plt.savefig(os.path.join(BASE_DIR, '19_3e_retornos_real_vs_predicho.png'), dpi=150, bbox_inches='tight')
plt.close()
print(f"  [OK] Guardado: 19_3e_retornos_real_vs_predicho.png")
print(f"       Correlacion real vs predicho (media diaria): {_corr_media:.4f}")
print(f"       RMSE media diaria: {_rmse_media:.6f}")
...
VENTANA_TRAIN_BT  = 252   # 1 año de datos para estimar mu y Sigma
PASO_REOPT        = 22    # Reoptimizar cada ~1 mes
COSTE_TRANSACCION = 0.001 # 10 bps por trade (ida y vuelta) (se evaluarán otras opciones)
...
print(f"    19_3e_retornos_real_vs_predicho.png        - Retornos reales vs predichos (media diaria)")
